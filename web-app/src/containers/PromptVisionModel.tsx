...
export function PromptVisionModel({
  ...
  const fetchJanV2VLModel = useCallback(async () => {
    ...
    try {
      ...
      console.error('Error fetching Overchat V2 VL Model:', error)
    } finally {
      setIsLoading(false)
    }
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 p-4 shadow-lg bg-background w-4/5 md:w-100 border rounded-lg">
      <div className="flex items-center gap-2">
        <img src="/images/overchat.png" alt="Overchat" className="size-5" />
        <h2 className="font-medium">
          Overchat V2 VL Model
          <span className="text-muted-foreground"> (~5GB)</span>
        </h2>
      </div>
      <p className="mt-2 text-sm text-muted-foreground">
        Add vision capabilities to chat with images. Download Overchat V2 VL, our
